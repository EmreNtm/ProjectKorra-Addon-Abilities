...
public class TerraSense extends EarthAbility implements AddonAbility, PassiveAbility {
	...
	public void senseEarthbending() {
		...
		if (Bukkit.getServer().getPluginManager().getPlugin("JedCore") == null)
			return;
	...
	public void load() {
		...
		packetTypes.add(PacketType.Play.Server.NAMED_ENTITY_SPAWN);
		myPacketAdapter = new MyPacketAdapter(ProjectKorra.plugin, packetTypes);
		ProtocolLibrary.getProtocolManager().addPacketListener(myPacketAdapter);
	...
	public void stop() {
		...
		ProtocolLibrary.getProtocolManager().removePacketListener(myPacketAdapter);
		super.remove();
	}
