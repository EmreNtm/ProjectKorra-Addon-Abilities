...
public class RagingFire extends FireAbility implements AddonAbility, ComboAbility {
	...
	public void progress() {
		...
		for (Location l : fireLocations.keySet()) {
			...
			for (double i = 0; i < fireLocations.get(l).distance(l); i+=1) {
				player.getWorld().spawnParticle(Particle.FLAME, tmpLoc.clone().add(0.2, 0.5, 0.5), 0);
				player.getWorld().spawnParticle(Particle.FLAME, tmpLoc.clone().add(0.8, 0, 0.2), 0);
				player.getWorld().spawnParticle(Particle.FLAME, tmpLoc.clone().add(0.8, 0.5, 0.8), 0);
				tmpLoc.add(0, 1, 0);
			}
