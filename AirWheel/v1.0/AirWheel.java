...
public class AirWheel extends AirAbility implements AddonAbility, ComboAbility {
	...
	public void startWheel() {
		for (int i = 0; i < 10; i++) {
			origin.setY(origin.getY() + 0.0075);
			angle = (float) (count * 137.5);
			radius = (float) (0.01 * Math.sqrt(angle));
	...
	public void stopWheel() {
		...
		radius = 0;
		for (int i = 0; i < 100; i++) {
			origin.setY(origin.getY() - 0.03);
			angle = (float) (count * 137.5);
			radius = (float) (0.02 * Math.sqrt(angle));
			origin.setX(origin.getX() + radius * Math.cos(angle));
			origin.setZ(origin.getZ() + radius * Math.sin(angle));
			getAirbendingParticles().display(origin, 0, 0, 0, 0, 5);
			hit(origin);
	...
	public void spinWheel() {
		...
		for (i = 0; i < 75; i++) {
			...
			location.setY(location.getY() + r * Math.sin(a) - 1);
			location.setZ(location.getZ() + r * direction.getZ() * Math.cos(a));
			getAirbendingParticles().display(location, 0, 0, 0, 0, 1);
			hit(location);
			hitBurst(location);
			n++;
		}
